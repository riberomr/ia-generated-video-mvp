...
export class BedrockService {
  ...
  async checkCompletion(config: {
    ...
    const {
      messages,
      model = "us.anthropic.claude-3-5-sonnet-20240620-v1:0", // Updated to cross-region profile as requested
      jsonMode = false,
    } = config;

    this.logger.log(`Calling Bedrock with model: ${model}`);

    try {
      // Prepare the payload for Claude 3 models
      // Claude 3 expects "messages" array in the body
      const payload: any = {
